...
def printMorseMessage(theInput):
  morseMessage = ""
  for i in range(len(theInput)):
    if theInput[i] in morseDict:
      morseMessage += morseDict[theInput[i]]
    elif theInput[i] == " ":
      morseMessage += " "
  return morseMessage

def createMusicalMorse(theInput):
  createdSong = []
  for i in range(len(theInput)):
    if theInput[i] == ".":
      createdSong.append(("c", 8,))
      createdSong.append(("r", 8))
    elif theInput[i] == "-":
      createdSong.append(("c", 2))
      createdSong.append(("r", 8))
    elif theInput[i] == " ":
      createdSong.append(("r", 2))
  pysynth.make_wav(createdSong, fn='Song.wav')

message = input("Type in a message to translate into morse code (in all lowercase letters): ")
print(printMorseMessage(message))
morseMessage = input("Type in a message to translate into morse music: ")
createMusicalMorse(morseMessage)